...
const Users = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await axios.get('https://event-backend-85661116f5a4.herokuapp.com/api/users');
        setUsers(res.data);
        setLoading(false);
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitting(true);
      const res = await axios.post('https://event-backend-85661116f5a4.herokuapp.com/api/auth/register', formData);
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`https://event-backend-85661116f5a4.herokuapp.com/api/users/${id}`);
